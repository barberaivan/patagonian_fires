...
ggsave("figures2/XX spatial patterns - fire by veg and topo - univariate.png",
       plot = fire_uni_topo,
       width = 16, height = 16.5, units = "cm")

fire_uni_pp <- ggarrange(plots = plist_uni[c(6, 5, 7, 8)], ncol = 4)
ggsave("figures2/XX spatial patterns - fire by veg and pp-ndvi-dist - univariate.png",
       plot = fire_uni_pp,
       width = 16, height = 16.5, units = "cm")
...
ggsave("figures2/S06) NDVI model predictions.png", plot = ndvi_plot_all,
       width = 16, height = 16.5, units = "cm")
...
mtopot <- readRDS("exports/models_topography.rds")
...
for(p in 1:P) {
  ...
  ## add model column
  dens_data$model <- factor("Joint model",
                            levels = c("Environmental\nmodel", "Joint model"))
  dens_data$model[dens_data$vegetation_class == "(1) All vegetation\ntypes"] <- "Environmental\nmodel"

  pred_local$model <- factor("Joint model",
                            levels = c("Environmental\nmodel", "Joint model"))
  pred_local$model[pred_local$vegetation_class == "(1) All vegetation\ntypes"] <- "Environmental\nmodel"

  effects_data_local$model <- factor("Joint model",
                            levels = c("Environmental\nmodel", "Joint model"))
  effects_data_local$model[effects_data_local$vegetation_class == "(1) All vegetation\ntypes"] <- "Environmental\nmodel"
  ##
  ...
  if(var == "pp") {
    plotcito <- plotcito +
    xlab(expression(paste("      (A) Precipitation\n", "(", mm~yr^{-1}, ")       ")))
    # xlab(expression(paste("(A) Precipitation ", "(", mm~yr^{-1}, ")")))
    # xlab(expression((A)~Precipitation~(mm~yr^{-1})))
    # xlab(as.list("(A) Precipitation", expression((mm~yr^{-1}))))
    # title <- as.list(expression(paste("Histogram of " , hat(mu)), "Bootstrap samples, Allianz"))
    #xlab(expression(Anthropogenic~SO[4]^{"2-"}~(ngm^-3))) +
  }
  ...
  if(var %in% c("TPI2k", "dist_roads")) {
    plotcito <- plotcito +
      theme(strip.background = element_rect(color = "white", fill = "white"),
            strip.text = element_text(size = 8, margin = margin(0.2, 0.01, 0.2, 0.01, unit = "mm" ))) +
      ggh4x::facet_nested_wrap(vars(model, vegetation_class),
                               ncol = 1, strip.position = "right",
                               axes = "all",
                               nest_line = element_line(linewidth = 0.3,
                                                        color = "gray"),
                               solo_line = T)
      # ggh4x::facet_wrap2(vars(vegetation_class), ncol = 1, strip.position = "right",
      #                    axes = "all",
      #                    strip = strip_themed(text_y = texts,
      #                                  background_y = backgrounds))
  }
...
ggsave("figures2/07) XX curves side by side_cov-topo-ndvi-dist.png",
       plot = vegs_side,
...
# ggsave("figures2/06) XX spatial effects summary - predictors_cov0.png",
#        width = 16, height = 14, units = "cm")
ggsave("figures2/06) XX spatial effects summary - predictors_cov-topo-ndvi-dist.png",
       width = 16, height = 14, units = "cm")
...
# ggsave("figures2/06) XX spatial effects summary - veg_cov0.png",
#        width = 16, height = 14, units = "cm")
ggsave("figures2/06) XX spatial effects summary - veg_cov-topo-ndvi-dist.png",
       width = 16, height = 14, units = "cm")
...
ggsave("figures2/S07) burn probability varying predictors or not.png",
...
#
# ggsave("figures2/XX predictors densities by veg type_cov-topo.png",
...
ggsave("figures2/06) veg_all.png", plot = veg_all,
...
ggsave("figures2/06) vegetation - conditional all types.png",
       width = 16, height = 14, units = "cm")
...
for(v in 1:V) {
  ...
  nn <- paste("figures2/", "pairs_by_veg_", v, ".png", sep = "")
