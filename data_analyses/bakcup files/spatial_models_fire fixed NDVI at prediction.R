...
ggsave("figures/05) spatial patterns - fire by veg and topo.png",
       plot = fire_uni_topo,
       width = 16, height = 16.5, units = "cm")

fire_uni_pp <- ggarrange(plots = plist_uni[c(6, 5, 7, 8)], ncol = 4)
ggsave("figures/06) spatial patterns - fire by veg and pp-ndvi-dist.png",
       plot = fire_uni_pp,
       width = 16, height = 16.5, units = "cm")
...
ggsave("figures/S06) spatial patterns - fire by veg and topo - mult reg.png", plot = fire_multi_topo,
       width = 16, height = 16.5, units = "cm")

fire_multi_pp <- ggarrange(plots = plist_multi[c(6, 5, 7, 8)], ncol = 4)
ggsave("figures/S07) spatial patterns - fire by veg and pp-ndvi-dist - mult reg.png", plot = fire_multi_pp,
       width = 16, height = 16.5, units = "cm")
...
ggsave("figures/XX multiple regression with and without NDVI.png",
...
panlabs$vegetation_class[panlabs$pp_class == "High"] <- "Steppe and\ngrassland"
panlabs$pfit <- 23.5
...
ggsave("figures/07) burn probability at common environments_B.png",
...
predictors_ndvi <- predictors[!predictors %in% c("ndvi_mean", "dist_human", "dist_roads")]
...
ggsave("figures/S08) NDVI model predictions.png", plot = ndviplot,
