...
firemod_marg_ref <- bam(
  ...
  data = data_coords, knots = list(aspect = c(0, 360)),
  family = "binomial",
...
# firemod_marg_sp <- readRDS("data_and_files/fire_model_gam_sp.rds")
...
newdata <- do.call("rbind", lapply(predictors, function(p) {
  # p = "elevation"
  if(p != "aspect") {
    rr <- make_newdata(p, data_coords[, predictors], HDI = TRUE, prob = 0.98)
  } else {
    rr <- make_newdata(p, data_coords[, predictors], HDI = FALSE)
  }
  return(rr)
}))

newdata$x <- mean(data_coords$x)
newdata$y <- mean(data_coords$y)

# non-spatial
lpmat <- predict(firemod_marg_ref, newdata, "lpmatrix")
coefs <- coef(firemod_marg_ref)
Sigma <- vcov(firemod_marg_ref, unconditional = T, freq = F)
se.fit <- apply(lpmat, MARGIN = 1, FUN = function(x) sqrt(t(x) %*% Sigma %*% x))
fit <- lpmat %*% coefs

pred_ref <- data.frame(p_mle = plogis(fit),
                       p_lower = plogis(fit - qnorm(0.975) * se.fit),
                       p_upper = plogis(fit + qnorm(0.975) * se.fit))
pred_ref$model <- "non-spatial"

# spatial
names(coef(firemod_marg_sp))
use <- 1:24
lpmat <- predict(firemod_marg_sp, newdata, "lpmatrix")[, use]
coefs_sp <- coef(firemod_marg_sp)[use]
# coefs_sp[1] <- coefs[1]
Sigma <- vcov(firemod_marg_sp, unconditional = T, freq = F)[use, use]
se.fit <- apply(lpmat, MARGIN = 1, FUN = function(x) sqrt(t(x) %*% Sigma %*% x))
fit <- lpmat %*% coefs_sp

pred_sp <- data.frame(p_mle = plogis(fit),
                      p_lower = plogis(fit - qnorm(0.975) * se.fit),
                      p_upper = plogis(fit + qnorm(0.975) * se.fit))
pred_sp$model <- "spatial"

# merge
pcomp <- rbind(
  cbind(newdata, pred_ref),
  cbind(newdata, pred_sp)
)

# plot
ggplot(pcomp, aes(varying_val, p_mle, ymin = p_lower, ymax = p_upper,
                  color = model, fill = model, group = model)) +
  geom_ribbon(color = NA, alpha = 0.4) +
  geom_line() +
  facet_wrap(vars(varying_var), scales = "free")

ggplot(pcomp[pcomp$model == "spatial", ],
       aes(varying_val, p_mle, ymin = p_lower, ymax = p_upper,
           color = model, fill = model, group = model)) +
  geom_ribbon(color = NA, alpha = 0.4) +
  geom_line() +
  facet_wrap(vars(varying_var), scales = "free_x")

mean(fitted(firemod_marg_sp))
mean(fitted(firemod_marg_ref))
mean(data_coords$burned)
plot(fitted(firemod_marg_sp) ~ fitted(firemod_marg_ref),
     col = rgb(0, 0, 0, 0.15), pch = 19)
abline(0, 1)

summary(firemod_marg_sp) # 50 % baja a 40 % con menos bases en sp
summary(firemod_marg_ref) # 15.1 %

# with spatial random effects, all effects disappear. CIs are small because the
# prediction is always zero.

coef_sp <- coef(firemod_marg_sp)[use]
coef_ref <- coef(firemod_marg_ref)
plot(coef_ref ~ coef_sp)

# the largest difference in effects is due to the low intercept that the
# spatial model estimates. if the same as the non-spatial is used,
# predictions are more alike.
# The IC width of the spatial model depends highly on the complexity of the
# spatial component. As it decreases, the model resembles the non-spatial,
# but even with k = 10, predictors effects were smaller.
