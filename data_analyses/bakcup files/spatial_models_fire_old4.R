...
for(p in 1:P) {
  ...
  ndvi_preds <- do.call("rbind", lapply(veg_model, function(veg) {
    ...
    ndvi_logit <- predict(ndvi_models_6[[veg]], ndvi_pred, se.fit = T)
...
# predictor > response > vegetation type
...
for(pred in topo) {
  ...
  for(resp in responses) {
    # resp = "elevation"
    mtopo[[pred]][[resp]] <- vector("list", 6) # vegetation types + all
    names(mtopo[[pred]][[resp]]) <- veg_model

    # loop over vegetation types to fit model
    for(veg in veg_model) {

      if(veg == veg_model[1]) {
        dataloc <- data
      } else {
        dataloc <- data[data$vegetation_class == veg, ]
      }

      if(resp == "slope") {
        dataloc$slope[dataloc$slope == 0] <- 0.0001
      }

      # model
      formula_row <- combs_topo$response == resp & combs_topo$predictor == pred
      mtopo[[pred]][[resp]][[veg]] <- gam(
        as.formula(combs_topo$formula[formula_row]), data = dataloc,
...
mtopo[["elevation"]][["slope"]][["Dry forest"]] %>% plot
mtopo[["elevation"]][["TPI2k"]][["Dry forest"]] %>% plot
...
for(v in 1:V) {
  ...
  for(predictor in topo) {
    ## get rows where this variable varies
    # predictor = "elevation"
    rows_varying <- newdata$varying_var == predictor
    # loop over the response variables that this predictor affects
    responses <- combs_topo$response[combs_topo$predictor == predictor]

    for(response in responses) {
      ## predict from the model
      # response = "slope"
      prediction <- predict(mtopo[[predictor]][[response]][[v]],
                            newdata = newdata[rows_varying, predictor, drop = F],
                            type = "response")
      newdata[rows_varying, response] <- prediction
    }
  }

  # once topographic features are resolved, predict NDVI.
  ...
  ndvi_pred <- predict(ndvi_models_6[[v]], newdata, type = "response")
  newdata$ndvi_mean[rows_ndvi_predict] <- ndvi_pred[rows_ndvi_predict]

  # predict fire probability
  fire_logit <- predict(mod, newdata, se.fit = T)

  pred_ppp <- data.frame(
    p_mle = plogis(fire_logit$fit),
    p_lower = plogis(fire_logit$fit - qnorm(0.975) * fire_logit$se.fit),
    p_upper = plogis(fire_logit$fit + qnorm(0.975) * fire_logit$se.fit)
  )

  # now do the same but fixing NDVI at its mean (no co-variation, fixed="f")
  ...
  # compute predictions
  fire_logit_f <- predict(mod, newdata_f, se.fit = T)
  pred_ppp_f <- data.frame(
    p_mle_f = plogis(fire_logit_f$fit),
    p_lower_f = plogis(fire_logit_f$fit - qnorm(0.975) * fire_logit_f$se.fit),
    p_upper_f = plogis(fire_logit_f$fit + qnorm(0.975) * fire_logit_f$se.fit))
  ...
  sims_lp <- sapply(1:ncol(dsim_local), function(j) {
    ...
    linpred_ran <- predict(mran, newdata, type = "link", se.fit = FALSE)
    return(linpred_ran)
  })
  ...
  for(p in predictors) {
    ...
    meanfit <- mean(fire_logit$fit[rr])
  ...
  # # check centering
  # rrr <- newdata$varying_var == "elevation"
  # plot(plogis(fire_logit$fit[rrr]) ~ newdata$elevation[rrr], type = "l")
  # for(i in 1:ncol(sims_lp)) {
  #   lines(plogis(sims_lp[rrr, i]) ~ newdata$elevation[rrr], col = rgb(1, 0, 0, 0.05))
  # }
...
saveRDS(firelist_cond, "data_and_files/fire_models_multivariate_varying-NDVI-at-prediction_2.rds")
firelist_cond <- readRDS("data_and_files/fire_models_multivariate_varying-NDVI-at-prediction_2.rds")


# Fitted values in pooled model -------------------------------------------

lapply(firelist_cond, function(x) {
  tt <- predict(x$model, type = "terms")
  sum(colMeans(tt))
})

# terms are not centred, so if some terms are not included, the average burn
# probability will be biased.
x <- firelist_cond[[1]]
tt <- predict(x$model, type = "terms")

coef(x$model)[1] %>% plogis # higher mean
mean(fitted(x$model)) # GOOD mean
(sum(colMeans(tt)) + coef(x$model)[1]) %>% plogis # lower mean

# So, removing terms from the model biases the average burn prob. It should not
# be so high!!
...
ggsave("figures/05) spatial patterns - fire by veg and topo - mult reg - varying NDVI_fixedy_3.png",
       plot = fire_multi_topo,
       width = 16, height = 16.5, units = "cm")

fire_multi_pp <- ggarrange(plots = plist_multi[c(6, 5, 7, 8)], ncol = 4)
ggsave("figures/06) spatial patterns - fire by veg and pp-ndvi-dist - mult reg - varying NDVI_fixedy_3.png",
       plot = fire_multi_pp,
       width = 16, height = 16.5, units = "cm")

# _2 is covariation of topography with ylim(0, 40)
# _3 is the same with ylim(0, 30)
...
for(p in 1:P) {
  ...
  plotcito <-
    ...
    geom_line(
      data = pred[is.na(pred$vegetation_class), ],
...
ggsave("figures/07) XX curves side by side_3.png",
       plot = vegs_side,
...
write.csv(delta_plot,
          "data_and_files/fire_models_multivariate_effects_summary_2.csv",
          row.names = F)
...
ggplot(delta_plot) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, 10), breaks = seq(0, 10, by = 2.5), expand = c(0.01, 0.01),
    sec.axis = sec_axis(~ . / q,
                        name = "p-value",
...
ggsave("figures/06) XX spatial effects summary - predictors_2.png",
...
ggsave("figures/06) XX spatial effects summary - veg_2.png",
       width = 16, height = 14, units = "cm")
...
for(p in 1:Pndvi) {
  ...
  # DO NOT FORCE THE SAME MEAN IF YOU WANT TO TRULY OBSERVE THE
  # EFFECT OF NDVI
  #
...
ggsave("figures/S07) burn probability varying NDVI or not_2.png",
