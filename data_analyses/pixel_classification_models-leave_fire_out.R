# Este script es igual a su homónimo, pero en vez de hacer un CV con 20 folds hago
# tantos folds como fuegos
...
nfires <- length(unique(train$Fire_ID))
k <- nfires
fold_id <- as.numeric(factor(train$Fire_ID))
...
ggplot(senspec_long, aes(x = threshold, y = senspec, colour = performance)) +
  ...
  ggtitle("Performance de modelos (Leave-Fire-Out)") +
...
# Productivity distribution by fire ---------------------------------------

ggplot(train, aes(ndvi_max)) +
  geom_density() +
  facet_wrap(vars(Fire_ID))

# Umbrales para semillas --------------------------------------------------
# (Código no editado; lo de abajo fue basado en el 20-fold independent CV,
# no en el leave-fire-out)
