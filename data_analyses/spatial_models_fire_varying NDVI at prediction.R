...
burned_veg$vegetation_class[burned_veg$vegetation_class == "Steppe and grassland"] <- "Steppe"
...
ggsave("figures/05) spatial patterns - fire by veg and topo - mult reg - varying NDVI.png",
       plot = fire_multi_topo,
       width = 16, height = 16.5, units = "cm")

fire_multi_pp <- ggarrange(plots = plist_multi[c(6, 5, 7, 8)], ncol = 4)
ggsave("figures/06) spatial patterns - fire by veg and pp-ndvi-dist - mult reg - varying NDVI.png",
       plot = fire_multi_pp,
       width = 16, height = 16.5, units = "cm")
...
# the difference is small, don't worry about it.

# Multivariate models with and without NDVI (not used) --------------------
...
pd$show <- factor(
  ifelse(
    (pd$vegetation_class == "Steppe" & pd$pp_class == "High") |
...
panlabs$vegetation_class[panlabs$pp_class == "High"] <- "Steppe"
panlabs$pfit <- 23.5
