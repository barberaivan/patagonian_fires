...
# vegmod <- gam(
#   list(veg_num ~ s(elevation, k = 5, bs = "cr") + s(slope, k = 5, bs = "cr") +
#                  s(aspect, k = 6, bs = "cc") + s(TPI2k, k = 5, bs = "cr") +
#                  s(pp, k = 5, bs = "cr") +
#                  s(dist_human, k = 5, bs = "cr") + s(dist_roads, k = 5, bs = "cr"),
#
#                ~ s(elevation, k = 5, bs = "cr") + s(slope, k = 5, bs = "cr") + # dry forest
#                  s(aspect, k = 6, bs = "cc") + s(TPI2k, k = 5, bs = "cr") +
#                  s(pp, k = 5, bs = "cr") +
#                  s(dist_human, k = 5, bs = "cr") + s(dist_roads, k = 5, bs = "cr"),
#
#                ~ s(elevation, k = 5, bs = "cr") + s(slope, k = 5, bs = "cr") +
#                  s(aspect, k = 6, bs = "cc") + s(TPI2k, k = 5, bs = "cr") +
#                  s(pp, k = 5, bs = "cr") +
#                  s(dist_human, k = 5, bs = "cr") + s(dist_roads, k = 5, bs = "cr"),
#
...
ndveg0 <- do.call("rbind", lapply(predictors, function(var) {
  make_newdata(var, data_veg = data_veg[, predictors])
}))
ndveg <- ndveg0
...
pred_long_ci <- as.data_veg.frame.table(apply(pred_arr, 1:2, ci))
head(pred_long_ci)
...
pred_long_mle <- as.data_veg.frame.table(prob_hat)
pred_long_mle <- cbind(data_veg.frame("Var1" = rep("p_mle", np)),
                       pred_long_mle)
...
ggsave("figures/spatial patterns - veg as a function of fire drivers.png",
       plot = vegplot_leg,
...
for(p in predictors) {
  ...
  # model settings
  kn = 5; bs = "cr"; knots = NULL
  if(p == "aspect") { kn = 6; bs = "cc"; knots = list(x = c(0, 360)) }
...
ggsave("figures/05) spatial patterns - vegetation drivers marginal.png",
...
ggsave("figures/S03) spatial patterns - vegetation drivers conditional.png",
       plot = vegsep_dens,
...
ggplot(data_veg, aes(x = pp, y = elevation)) +
...
ggplot(data_veg, aes(x = pp, y = elevation, color = vegetation_class,
